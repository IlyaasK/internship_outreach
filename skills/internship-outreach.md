...
Proof:
- [Video analysis project](https://github.com/nyle06dn/VideoAnalysis) and [automated temperature alert](https://github.com/nyle06dn/BATTERY_Auto_Switch) for Bearcat Motorsports FSAE Formula 1 Electric Vehicle Student Team.
...
### Step 2.5 — Find the CEO / Founder's Email via Exa AI Search

This step is required for every company. Use `exa_search` to attempt to locate a direct email for the founder before sending.

**Search query to use (in order of preference):**
1. `"[Founder Full Name]" "[Company Name]" email` — direct match
2. `"[Founder Full Name]" site:[companydomain.com] email` — domain-scoped
3. `"[Founder Full Name]" "[Company Name]" contact OR reach OR @` — fallback

**How to execute:**
- Use `exa_search` with `type: "keyword"` and `numResults: 5`.
- Scan result snippets and URLs for a visible email address (format: `something@domain.com`).
- If an email is found, record it. If **not** found, record `"Email not found"` — do not guess or fabricate.

**Do NOT:**
- Guess email formats (e.g. `firstname@company.com`) unless you found it explicitly confirmed in a source.
- Use the company's generic `contact@` or `hello@` addresses — those are not the founder's personal email.
...
**Email:** [founder@company.com, or "Email not found"]
**YC:** [Yes — Batch / No]
...
| # | Company | Founder | Email | YC? | Milestone Found? |
|---|---|---|---|---|---|
| 1 | ... | ... | ... | ... | ... |
